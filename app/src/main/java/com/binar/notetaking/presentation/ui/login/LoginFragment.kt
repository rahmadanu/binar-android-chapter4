...
class LoginFragment : Fragment() {
        ...
        if (validateInput()){
            ...
            viewModel.getUserResult.observe(viewLifecycleOwner) {
                when (it) {
                    ...
                    is Resource.Error -> Toast.makeText(requireContext(), it.exception?.message.toString(), Toast.LENGTH_SHORT).show()
                    else -> {}
                }
            }
        }
    ...
    private fun navigateToHome() {
        val option = NavOptions.Builder()
            ...
            .build()
        val action = LoginFragmentDirections.actionLoginFragmentToNoteFragment(args)
